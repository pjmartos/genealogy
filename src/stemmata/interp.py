...
def interpolate(
    ...
    *,
    ...
    namespace = tree
    return _interp(tree, namespace, layers, parent_is_list=False, root_file=root_file, visiting=())
...
def _interp(node: Any, namespace: Any, layers: list[Layer], *, parent_is_list: bool, root_file: str, visiting: tuple[str, ...]) -> Any:
    if isinstance(node, dict):
        return {k: _interp(v, namespace, layers, parent_is_list=False, root_file=root_file, visiting=visiting) for k, v in node.items()}
    if isinstance(node, list):
        out: list[Any] = []
        for item in node:
            resolved = _interp(item, namespace, layers, parent_is_list=True, root_file=root_file, visiting=visiting)
